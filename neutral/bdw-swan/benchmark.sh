...
case "$COMPILER" in
    ...
    gcc-7.3)
        ...
        MAKE_OPTS="$MAKE_OPTS"' CFLAGS_GCC="-std=gnu99 -Wall -fopenmp -Ofast -march=broadwell -ffast-math -ffp-contract=fast"'
        ;;
    intel-2018)
        module swap PrgEnv-{cray,intel}
        module swap intel intel/18.0.0.128
        MAKE_OPTS='COMPILER=INTEL ARCH_COMPILER_CC=cc ARCH_COMPILER_CPP=CC'
        MAKE_OPTS="$MAKE_OPTS"' CFLAGS_INTEL="-O3 -qopenmp -no-prec-div -std=gnu99 -DINTEL -Wall -xCORE-AVX2"'
        ;;
    *)
        echo

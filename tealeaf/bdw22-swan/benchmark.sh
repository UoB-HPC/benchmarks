...
function setup_env()
{
  ...
  case "$COMPILER" in
      cce-9.0)
          ...
          MAKE_OPTS=$MAKE_OPTS' CFLAGS_CRAY="-Ofast -fopenmp -march=broadwell -funroll-loops"'
          ;;
      gcc-8.3)
          module swap $PRGENV PrgEnv-gnu
          module swap gcc gcc/8.3.0
          MAKE_OPTS='COMPILER=GNU MPI_COMPILER=ftn C_MPI_COMPILER=cc'
          MAKE_OPTS=$MAKE_OPTS' FLAGS_GNU="-Ofast -march=broadwell -funroll-loops -cpp -ffree-line-length-none"'
